...
static void chunk_output(EAGLESystems *arg, uint8_t *buf, size_t len)
{
    uint32_t off = 0;
    uint8_t tmp[512];
...
static void onRxPacket(
    ...
    void *userData)
{
    FskReassemblyState *s = static_cast<FskReassemblyState *>(userData);
    ...
    fsk_reassembler_ingest(s, buf, (uint32_t)millis());
}
...
void setup()
{
    ...
    // 10 s cold-start window — same as benchmark
    driver.receiveOpen(
        PAYLOAD_SIZE_FSK,
        onRxPacket,
        &rxState,
    ...
    if (rxState.stat_frags_total == 0 && rxState.stat_crc_drop == 0)
    {
    ...
    Serial.println(F("[EAGLE] First packet received! Starting reassembly..."));

    // Reset for the real timed window — same pattern as benchmark ctx reset
    rxState.stat_frags_total = 0;
    rxState.stat_crc_drop = 0;
    rxState.stat_frame_resets = 0;
    fsk_reassembler_reset(&rxState);

    unsigned long t0 = micros();

    // 500 ms idle = burst done, matching benchmark convention
    ...
    uint32_t bytesRx = rxState.stat_frags_total * FSK_FRAG_DATA_SIZE;
    ...
    Serial.println(rxState.stat_frags_total);
    Serial.print(F("  CRC drops       : "));
    Serial.println(rxState.stat_crc_drop);
    ...
    // Stream reassembled frame out over USB serial
    uint32_t frameSize = rxState.completed_size > 0 ? rxState.completed_size : IMAGE_SIZE;

    EAGLESystems eagle;
    eagle.serial = reinterpret_cast<HardwareSerial *>(&Serial);

    Serial.println(F("[EAGLE] Streaming reassembled frame over serial..."));
    Serial.flush();

    chunk_output(&eagle, rxState.data_buf, frameSize);
