...
//     // ── Single IDF SPI bus init — owns the bus exclusively ──
...
//     // No SPICOMMON_BUSFLAG_MASTER — not needed, master is the default
//     esp_err_t busErr = spi_bus_initialize(SPI2_HOST, &buscfg, SPI_DMA_CH_AUTO);
//     if (busErr != ESP_OK)
//     {
//         Serial.print(F("SPI bus init failed: "));
//         Serial.println(esp_err_to_name(busErr));
//         while (true)
//         {
//             delay(10);
//         }
...
//     // Attach Arduino SPIClass to the already-initialized IDF bus so RadioLib's
//     // internal calls (beginFLRC, fixedPacketLengthMode, etc.) work normally.
//     // Passing -1 for CS prevents SPIClass from managing CS itself.
//     // Arduino ESP32 core detects the bus is already up and skips spi_bus_initialize().
//     mySPI.begin(SPI_SCK, SPI_MISO, SPI_MOSI, -1);
...
//             // NOTE: spiHandle not yet initialized (initSpiDma() called later), so we
//             // must use mySPI here. This is safe — mySPI and spiHandle are sequential,
//             // never concurrent. initSpiDma() is called after all setup SPI commands.
//             mySPI.beginTransaction(spiSettings);
//             digitalWrite(LR2021_CS, LOW);
//             mySPI.transferBytes(configFifoCmd, nullptr, sizeof(configFifoCmd));
//             digitalWrite(LR2021_CS, HIGH);
...
//         // DS Table 5-17: TxDone = bit 19, TxFifoIrq = bit 17 (verify in your datasheet §5.7)
//         {
...
//             // digitalWrite(LR2021_CS, HIGH);
//             // mySPI.endTransaction();
//             // Same as above: spiHandle not yet ready, mySPI used intentionally here.
//             mySPI.beginTransaction(spiSettings);
//             digitalWrite(LR2021_CS, LOW);
//             mySPI.transferBytes(setDioIrqCmd, nullptr, sizeof(setDioIrqCmd));
//             digitalWrite(LR2021_CS, HIGH);
...
//     // initSpiDma() registers spiHandle on the IDF bus. Must be called AFTER all
//     // mySPI-based setup commands above (ConfigFifoIrq, SetDioIrqConfig) so those
//     // commands go through mySPI without contending with spiHandle.
//     dmaReady = initSpiDma();
...
//     payload[0] = 0;
//     payload[1] = 0;
//     // Phase 1: WriteRadioTxFifo — first 255 bytes
//     // {
...
//     //   mySPI.endTransaction();
//     // }
//     // DMA write — chunk 1 of first packet
//     // dmaBuf1 headers already set in initSpiDma(); only payload bytes need copying
//     while (digitalRead(LR2021_BUSY))
//     {
//     }
//     dmaWriteTxFifo(dmaBuf1, payload, FIFO_CHUNK_1, DMA_BUF1_SIZE);
...
//         // digitalWrite(LR2021_CS, HIGH);
//         // mySPI.endTransaction();
//         spiSend(setTxCmd, sizeof(setTxCmd));
//     }
//     packetsSent = 1;
//     benchStartMs = millis();
// #endif
...
//             // digitalWrite(LR2021_CS, HIGH);
//             // mySPI.endTransaction();
//             spiSendRecv(irqCmd, irqResp, sizeof(irqCmd));
...
//             // ── TxFifoLow IRQ: FIFO has drained enough — write second chunk ──
//             // TxFifoIrq is bit 17 in LR2021 IRQ table (DS §5.7)
...
//                 // digitalWrite(LR2021_CS, HIGH);
//                 // mySPI.endTransaction();
//                 // Radio continues transmitting — TxDone IRQ will fire when packet is complete

//                 while (digitalRead(LR2021_BUSY))
//                 {
//                 }
//                 dmaWriteTxFifo(dmaBuf2, fifoRefillPtr, fifoRefillLen, DMA_BUF2_SIZE);
//             }

//             // ── TxDone IRQ: packet fully transmitted ──
//             // TxDone is bit 19 in LR2021 IRQ table (DS §5.7 / Table 5-17)
//             if (irqStatus & (1UL << 19))
//             {
//                 packetsOk++;
...
//                 // ── Transmit next packet — two-phase FIFO write ──
...
//                 // Arm the refill — ISR will set radioEvent, loop() writes chunk 2
//                 fifoRefillPtr = payload + FIFO_CHUNK_1;
//                 fifoRefillLen = FIFO_CHUNK_2;
//                 fifoRefillNeeded = true;

//                 // Phase 2: SetTx — radio starts transmitting immediately from FIFO
...
//         // Timeout watchdog — same logic as before
//         static unsigned long lastEventMs = 0;
//         if (packetsSent > 0 && millis() - lastEventMs > 500)
//         {
//             packetsFailed++;
//             // Force a restart — just re-invoke the first packet of next sequence
//             lastEventMs = millis();
//         }
//     }
//     else
...
//         {
//             // CAM is done transmitting but we never got all packets — force terminate
...
//             // buffer only — NO serial prints here
